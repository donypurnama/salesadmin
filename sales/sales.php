...
<?php
	...
	if ($update == 1)
	{
		...
		$commdsgroupcode = $_POST['commdsgroupcode'];
		$commtrgroupcode = $_POST['commtrgroupcode'];
		...
		{	//UPDATE FAKTUR
			...
			$queryupd = "update tb_invoice 
									set 
										...
										percentds		='".$commdsgroupcode."',
										percenttr		='".$commtrgroupcode."',
										trainer			='".$trainer."', 
	...
	$commdsgroupcode = $row['percentds'];
	$commtrgroupcode = $row['percenttr'];
...
if ($commdsgroupcode <> "") {
		...
		$commdsstr = $rowdiv['divisionname'];
		$rscomm = read_write("select * from tb_commrules where commgroupcode='".$commdsgroupcode."'");
		while ($rowcomm = mysql_fetch_array($rscomm)) {
			$startdate = $rowcomm['startdate'];
			$enddate = $rowcomm['enddate'];
			$commdsstr = $commdsstr."<br>".$startdate." - ".$enddate.": ".$rowcomm['percent_comm']." %";
		}
		$commdsstr = $commdsstr."<br>> ".$enddate.": 0<br>";
	}
if ($commtrgroupcode <> "") {
		$rsdiv = read_write("select divisionname from tb_division where divisionid='".$divisionid."'");
		$rowdiv = mysql_fetch_array($rsdiv);

		$commtrstr = $rowdiv['divisionname'];
		$rscomm = read_write("select * from tb_commrules where commgroupcode='".$commtrgroupcode."'");
		while ($rowcomm = mysql_fetch_array($rscomm)) {
			$startdate = $rowcomm['startdate'];
			$enddate = $rowcomm['enddate'];
			$commtrstr = $commtrstr."<br>".$startdate." - ".$enddate.": ".$rowcomm['percent_comm']." %";
		}
		$commtrstr = $commtrstr."<br>> ".$enddate.": 0<br>";
	}
	...
	<title>Add/Edit Invoice</title>
		...
		<script language="javascript">
			...
			function opencommission() {
				...
				window.open('commission.php?section=sales&commgroupcode='+scommgroupcode+'&divisionid='+sdivisionid+'&search=1','mywin','scrollbars=1,width=550,height=450,resizable=1');
				
			}
			function opendscommission() {
				var scommdsgroupcode = document.forms[0].commdsgroupcode.value;
				var sdivisionid = document.forms[0].divisionid.value;
				window.open('commission.php?section=ds&commgroupcode='+scommdsgroupcode+'&divisionid='+sdivisionid+'&search=1','mywin','scrollbars=1,width=550,height=450,resizable=1');
				
			}
			function opentrcommission() {
				var scommtrgroupcode = document.forms[0].commtrgroupcode.value;
				var sdivisionid = document.forms[0].divisionid.value;
				window.open('commission.php?section=tr&commgroupcode='+scommtrgroupcode+'&divisionid='+sdivisionid+'&search=1','mywin','scrollbars=1,width=550,height=450,resizable=1');
		...
		<tr>
			...
			<?} ?>			
				...
				<tr valign=top class="small">
					...
					<tr valign=top class="small">
					<td align="right" >Overriding DS Type:</td>
					<td><div id=commdstype><?php echo $commdsstr; ?></div><a href='javascript:opendscommission()' class=smalllink>Select type</a><input type=hidden name=commdsgroupcode value=<?php echo $commdsgroupcode;?>>
					<input type=hidden name=dbcommdsgroupcode value=<?php echo $commdsgroupcode;?>>
					</td>					
				</tr>
				<tr valign=top class="small">
					<td align="right" >Overriding TR Type:</td>
					<td><div id=commtrtype><?php echo $commtrstr; ?></div><a href='javascript:opentrcommission()' class=smalllink>Select type</a><input type=hidden name=commtrgroupcode value=<?php echo $commtrgroupcode;?>>
					<input type=hidden name=dbcommtrgroupcode value=<?php echo $commtrgroupcode;?>>
					</td>					
				</tr>
				<tr>
