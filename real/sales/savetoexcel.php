...
if ($ver==1) { // Branch Report
	...
	elseif ($ver==2) //Sales Book Report
{	
	...
	elseif ($ver ==3) //Export To Excel Product report
{
	...
	elseif ($ver ==4)  //Export to Salesman Report
{
	$res = read_write($_SESSION['queryinv']);
...
while ($row = mysql_fetch_array($res)) 
{
		...
		elseif ($ver==2) // Salesbook Report
	{
		...
		elseif ($ver==3) // Export Product Report
	{
		$invoiceno = trim($row['invoiceno']);	
		...
		$rsitems = read_write("select 
									tb_product.unit, 
									tb_product.productname, 
									tb_product.volume,  
								(tb_invoiceitems.qty-tb_invoiceitems.qty_return) as qty, 
								...
								from 
									...
									tb_product 
								where 
									tb_invoiceitems.invoiceno='".$invoiceno."' 
								and 
		...
		while ($rowitems = mysql_fetch_array($rsitems)) {
			...
			xlsWriteLabel($no_row,3,$rowitems['productname']." ".$rowitems['otherproductname']);
