...
<?
	...
	if($update==1) 
	{
		...
		for($i=1; $i<=$jumprod; $i++) 
		{
			...
			$_SESSION['productname'] = mysql_real_escape_string($_POST['productname'.$i]);
			$_SESSION['volume']	     = $_POST['volume'.$i];
			$_SESSION['pcs']		 = $_POST['pcs'.$i];
			$_SESSION['unit']	     = $_POST['unit'.$i];
			$_SESSION['description'] = mysql_real_escape_string($_POST['description'.$i]);
