...
include('../database.php');
	...
	if ($update <> 1) {
		if ($act == 2) {
			...
			Header("Location: viewcommission.php?commgroupcode=".$commgroupcode."");
		}
	} else {
		...
		if ($act == 1) {
			read_write("insert into tb_commrules (
							startdate, 
							enddate, 
							percent_comm, 
							commgroupcode) 
			...
			read_write("update tb_commrules set 
							startdate=".$txtstart.", 
							enddate=".$txtend.", 
...
<form name=frmcomm action="editcommission.php?update=1&act=<?php echo $act;?>&commruleid=<?php echo $commruleid;?>&commgroupcode=<?php echo $commgroupcode; ?>&divisionid=<?php echo $divisionid;?>" method=post>
	<?php if ($act==4) { ?>
...
<input type=button value='Cancel' class=tBox onclick="location.href='commission.php?commgroupcode=<?php echo $commgroupcode;?>'">
&nbsp;<input type=button value='Back' class=tBox onclick='javascript:location.href="commission.php?divisionid=<?echo $divisionid;?>&search=1"'>&nbsp;
</td></tr>
</table>
</form>
<?php
