...
include('../database.php'); 
	...
	$commgroupcode = $_GET['overgroupcode'];
	$update = $_GET['update'];
	...
	$query = "UPDATE 
						tb_commgroup 
					SET 
						overgroupcode 	='".stripslashes($_POST['commgroupcode'])."',
	...
	$query=$query." WHERE overgroupcode='".$commgroupcode."'";
		read_write($query);
	...
	if ($del==1) {
		$divisionid = $_GET['divisionid'];
		read_write("DELETE FROM tb_commgroup where overgroupcode='".$commgroupcode."'");
		Header("Location: comm-overiding.php?search=1&divisionid=".$divisionid);
	}
	$res = read_write("select * from tb_commgroup where overgroupcode='".$commgroupcode."'");
	...
	$res_a = read_write("select 
						...
						and 
							tb_commgroup.overgroupcode='".$commgroupcode."'");
	/*---------------------------------------------------------------------------------------------------------------*/							
	...
	<link rel="stylesheet" type="text/css" href="../templates/css/style.css">
		...
		function deletecomm(commruleid, commgroupcode) {
			if (confirm("Are you sure to delete this commission rule?")) {
				location.href='editoverriding.php?act=3&commruleid=' + commruleid + '&commgroupcode=' + commgroupcode;
			}
		}
		</script>
...
<form name=frmcomm action="viewoverriding.php?update=1&overgroupcode=<?php echo $commgroupcode;?>" method=post>
...
<tr class=small>
	...
	<td><input type=text class=tBox size=10 name='commgroupcode' value='<?php echo $row['overgroupcode'];?>'> </td>
</tr>
<tr class=small>
...
<tr>
	...
	&nbsp;<input type=button value='Back' class=tBox onclick='javascript:location.href="comm-overiding.php?divisionid=<?echo $row['divisionid'];?>&search=1"'>&nbsp;
	</td>
</tr>
</table>
</form><br>
...
?>
	...
	<td>&nbsp;</td></tr>
<?php	$res = read_write("select * from tb_overrules where overgroupcode='".$commgroupcode."'");
		while ($row = mysql_fetch_array($res)) {
			echo "<tr class=small>";
			echo "<td align=center>".$row['startdate']."</td><td align=center>".$row['enddate']."</td>";
			echo "<td align=right>".$row['percent_comm']." &nbsp;</td>";
			echo "<td align='center'><a href='editoverriding.php?act=2&commruleid=".$row['commruleid']." &overgroupcode=".$commgroupcode."'  class=smalllink >Edit</a> 
...
?>
	...
	<tr class=small>
	<td colspan=4><a href='editoverriding.php?act=1&commgroupcode=<?php echo $commgroupcode; ?>&divisionid=<? echo $divisionid;?>' class=smalllink>Add New Commission Rule</a></td>
	</tr>
	</table>
</body>
</html>
