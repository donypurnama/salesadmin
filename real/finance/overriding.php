...
while ($rwds = mysql_fetch_array($resds)) {
	...
	mysql_free_result($reshit);
		...
		$reshit = read_write("select sum(comm_ds) as commds  from ".$_SESSION['sqlfrom']."  where ".$_SESSION['sqlwhere']."  and tb_invoice.district_supervisor='".$rwds['district_supervisor']."'");
		$rwhit = mysql_fetch_array($reshit);
		$commtn = $rwhit['commds'];
		mysql_free_result($reshit);
		...
		if ($idx > 0) {
			foreach ($arrsales as $slcode) {
				...
				if ($j==count($arrsales)) {
					$commstr .= "<td align=right>".number_format($total_coll)." &nbsp;</td><td align=center>".$percent_ds."</td>";
...
while ($rwds = mysql_fetch_array($resds)) {
	...
	if ($active=="1" && $position=="2") {
		$reshit = read_write("select sum(comm_trainer) as commtn  from ".$_SESSION['sqlfrom']."  where ".$_SESSION['sqlwhere']."  and tb_invoice.trainer='".$rwds['trainer']."'");
		...
		if ($idx > 0) {
			foreach ($arrsales as $slcode) {
				...
				if ($j==count($arrsales)) {
					$commstr .= "<td align=right>".number_format($total_coll)." &nbsp;</td><td align=center>".$percent_trainer."</td>";
					$commstr .= "<td align=right>".number_format($commtn)." &nbsp;</td>";
				} else {
					$commstr .= "<td colspan=3></td>";
				}		
