...
include('../database.php'); 
	...
	if ($del==1) {
		$divisionid = $_GET['divisionid'];
		read_write("DELETE FROM tb_commgroup where overdsgroupcode='".$commgroupcode."'");
		Header("Location: comm-overidingds.php?search=1&divisionid=".$divisionid);
	}
	$res = read_write("select * from tb_commgroup where overdsgroupcode='".$commgroupcode."'");
	...
	$res_a = read_write("select 
						...
						and 
							tb_commgroup.overdsgroupcode='".$commgroupcode."'");
	/*---------------------------------------------------------------------------------------------------------------*/							
...
<html>
	...
	<link rel="stylesheet" type="text/css" href="../templates/css/style.css">
		...
		function deletecomm(overdsruleid, overdsgroupcode) {
			if (confirm("Are you sure to delete this overiding rule?")) {
				location.href='editoverridingds.php?act=3&overdsruleid=' + overdsruleid + '&overdsgroupcode=' + overdsgroupcode;
			}
		}
		</script>
...
<form name=frmcomm action="viewoverridingds.php?update=1&overdsgroupcode=<?php echo $commgroupcode;?>" method=post>
...
<tr class=small>
	...
	<td><input type=text class=tBox size=10 name='commgroupcode' value='<?php echo $row['overdsgroupcode'];?>'> </td>
</tr>
<tr class=small>
...
<tr>
	...
	&nbsp;<input type=button value='Back' class=tBox onclick='javascript:location.href="comm-overidingds.php?divisionid=<?echo $row['divisionid'];?>&search=1"'>&nbsp;
	</td>
</tr>
</table>
</form><br>
...
?>
	...
	<td>&nbsp;</td></tr>
<?php	$res = read_write("select * from tb_overdsrules where overdsgroupcode='".$commgroupcode."'");
		while ($row = mysql_fetch_array($res)) {
			echo "<tr class=small>";
			echo "<td align=center>".$row['startdate']."</td><td align=center>".$row['enddate']."</td>";
			echo "<td align=right>".$row['percent_comm']." &nbsp;</td>";
			echo "<td align='center'><a href='editoverridingds.php?act=2&overdsruleid=".$row['overdsruleid']." &overdsgroupcode=".$commgroupcode."'  class=smalllink >Edit</a> 
			
			&nbsp;<a href='javascript:deletecomm(".$row['overdsruleid'].",\"".$commgroupcode."\")' class=smalllink>Delete</a></td></tr>";
...
?>
	...
	<tr class=small>
	<td colspan=4><a href='editoverridingds.php?act=1&overdsgroupcode=<?php echo $commgroupcode; ?>&divisionid=<? echo $divisionid;?>' class=smalllink>Add New Commission Rule</a></td>
	</tr>
	</table>
</body>
</html>
