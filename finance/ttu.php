...
<?php
	...
	if ($update == 1) {
		...
		if ($payment >= 0 && ($outstanding-$payment)>=0) {
				...
				//$comm_ds = 0;
					...
					$resds = read_write("select 
							...
							from 
								tb_commrules 
							where 
								commgroupcode='".$commdsgroupcode."' 
							and 
								(".$datediff. "-startdate)>=0 
							and 
								(enddate-".$datediff.")>=0");
			if (mysql_num_rows($res) > 0) {
				$rspc = mysql_fetch_array($resds);
				$percent_ds = $rspc['percent_ds'];
				$comm_ds = $payment * $percent_ds/100;
			...
			if (mysql_num_rows($res) > 0) {
				$rspc = mysql_fetch_array($resds);
				$percent_trainer = $rspc['percent_trainer'];
				$comm_trainer = $payment * $percent_trainer/100;
					...
					{ //AUTO NUMBERIN FOR ADD TTU  //ADD TTU BARU
						...
						if (strlen($ttutemp)==1) {
							...
							$res = read_write("insert into tb_ttu (invoiceno, ttuno, ttudate, lpudate, depositdate,
								...
								/*$ttuid = mysql_insert_id();
echo "insert into tb_ttu (invoiceno, ttuno, ttudate, lpudate, depositdate,
																payment, ppn_payment, commission, percent_comm, percent_trainer, 
																comm_trainer, percent_ds, comm_ds,last_update,usercode, sentstatus) 
									values (
										'".$sid."', '".$ttuno."', '".$ttudate."', '".$lpudate."',
	...
	if ($ttuid <>'') {
		$res = read_write("select tb_ttu.*,
							...
							as 
								...
								tb_invoice.percentds,
								tb_invoice.percenttr,
...
<input type=hidden name=commdsgroupcode value=<?php echo $commdsgroupcode;?>>
<input type=hidden name=commtrgroupcode value=<?php echo $commtrgroupcode;?>>
...
<table border=0 cellspacing=1 cellpadding=1 width="40%" align=center>
	...
	<tr>
		...
		<tr valign=top class="small">
			...
			<td><?php echo $commdsstr; ?></td>					
		</tr>
			<tr valign=top class="small">
			<td align="right" >Overriding TR Type:</td>
			<td><?php echo $commtrstr; ?></td>					
		</tr>
		<?php
		if($ttuno <> "") {
