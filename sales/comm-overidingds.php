...
$overdsgroupcode = $_GET['overdsgroupcode'];
...
if ($choose == 1) {
	...
	$res = read_write("select * from tb_overdsrules where overdsgroupcode = '".$rdocomm."'");
	...
	echo "window.opener.document.forms[0].overdsgroupcode.value='".$rdocomm."';";
	echo "window.opener.document.getElementById('overdstype').innerHTML='".$commstr."';";
	echo "window.close();";
	echo "</script>";
}
...
?>
<html>
<head><title>Overriding DS</title>
<link rel="stylesheet" type="text/css" href="../templates/css/style.css">
<script language=javascript>
	function choosediv() {
		var division = document.forms[0].seldivision.value;		
		location.href='comm-overidingds.php?divisionid='+division+'&search=1';
	}
</script>
...
<body>
<form name=frmadd method=post action="comm-overidingds.php?choose=1">
...
if ($search == 1 && $divisionid <> '' && $divisionid <> '0') {
	...
	/*COUNT LIMIT */
	$res = read_write("select overdsgroupcode, implement_date, commgroupname from tb_commgroup where divisionid='".$divisionid."' order by rank desc");
	...
	if ($othercomm == 'all' || $overdsgroupcode <> '') {
		/* ALL COMMISSION SHOW */	
		$x = $jumcomm;	
		...
		while ($row = mysql_fetch_array($res)) {
			...
			echo "<tr class=small bgcolor='#e5ebf9' class=header>
				<td colspan=2 >
					<input type=radio name=rdocomm value=".$row['overdsgroupcode']; 
						if ($overdsgroupcode == $row['oversdgroupcode']) { echo " checked"; }
					echo "> &nbsp;".$row['commgroupname']." </td>";
			echo "</tr>";
			
			$rs = read_write("select * from tb_overdsrules where overdsgroupcode='".$row['overdsgroupcode']."' order by startdate");
	...
	/* ALL COMMISSION SHOW */		
		...
		$commgroupcode_default = $row_a['overdsgroupcode'];
		
		$res_b = read_write("select overdsgroupcode, 
									implement_date, 
									commgroupname  
							from 
								tb_commgroup 
							where 
								overdsgroupcode='".$commgroupcode_default."' 
							order by 
								rank desc 
							limit ".$x."");
		...
		while ($row = mysql_fetch_array($res)) {
			...
			echo "<tr class=small bgcolor='#e5ebf9' class=header>
				<td colspan=2 >
					<input type=radio name=rdocomm value=".$row['overdsgroupcode']; 
						if ($overdsgroupcode == $row['overdsgroupcode']) { 
							echo " checked"; 
						}
		...
		echo "<br><div align=center><a href='comm-overidingds.php?divisionid=".$seldivision."&search=1&othercomm=all'>Other Commission Type
