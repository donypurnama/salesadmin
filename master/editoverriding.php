...
include('../database.php');
	...
	if ($update <> 1) {
		if ($act == 2) {
			...
			Header("Location: viewoverriding.php?overgroupcode=".$commgroupcode."");
		}
	} else {
		...
		if ($act == 1) {
			read_write("insert into tb_overrules (
							startdate, 
							enddate, 
							percent_comm, 
							overgroupcode) 
			...
			read_write("update tb_overrules set 
							startdate=".$txtstart.", 
							enddate=".$txtend.", 
...
<form name=frmcomm action="editoverriding.php?update=1&act=<?php echo $act;?>&commruleid=<?php echo $commruleid;?>&commgroupcode=<?php echo $commgroupcode; ?>&divisionid=<?php echo $divisionid;?>" method=post>
	<?php if ($act==4) { ?>
...
<input type=button value='Cancel' class=tBox onClick="location.href='comm-overiding.php?commgroupcode=<?php echo $commgroupcode;?>'">
&nbsp;<input type=button value='Back' class=tBox onclick='javascript:location.href="comm-overiding.php?divisionid=<?echo $divisionid;?>&search=1"'>&nbsp;
</td></tr>
</table>
</form>
<?php
